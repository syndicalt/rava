...
fn functional_roadmap_separates_current_state_from_future_work() -> Result<(), Box<dyn Error>> {
    ...
    for required in [
        ...
        "https://github.com/syndicalt/rava/issues/118",
        "https://github.com/syndicalt/rava/issues/119",
        "https://github.com/syndicalt/rava/issues/120",
        "https://github.com/syndicalt/rava/issues/121",
        "https://github.com/syndicalt/rava/issues/122",
        "https://github.com/syndicalt/rava/issues/123",
        "https://github.com/syndicalt/rava/issues/124",
        "https://github.com/syndicalt/rava/issues/125",
...
fn external_review_outreach_tracker_records_reviewer_contact_without_audit_claims(
    ...
    for required in [
        ...
        "Report constraints",
        "Next action",
        "OpenSSF security-reviews public issue",
        "https://github.com/ossf/security-reviews/issues/102",
        "Public zero-budget community review/guidance request sent with frozen R4 target and review-packet links.",
        "Do not post undisclosed vulnerabilities in the public OpenSSF issue.",
        "Wait for OpenSSF maintainer or community response; intake any response before moving to `scoping`.",
        "## Intake Rules",
        "docs/security/external-review-request-v0.md",
        "docs/security/review-register-v0.md",
...
fn external_review_selection_rubric_records_reviewer_fit_without_endorsement_claims(
    ...
    for required in [
        ...
        "Decision",
        "OpenSSF security-reviews public issue",
        "Community channel can provide public written review guidance or redirect to an appropriate OpenSSF venue.",
        "Public issue; do not post undisclosed vulnerabilities; no production-ready or audited language.",
        "Selected for first zero-budget outreach attempt on 2026-05-18.",
        "## Non-Claim Boundary",
        "No production-ready or externally audited claim",
...
fn v0_review_candidate_notes_record_frozen_target_without_release_claims(
    ...
    for docs in [packet, checklist] {
        assert!(
            ...
            "review packet and checklist must link historical review candidate notes: {notes_path}"
        );
    }
...
fn v0_review_candidate_r2_records_historical_review_readiness_target() -> Result<(), Box<dyn Error>>
{
    ...
    for required in [
        ...
        "npm --prefix packages/rava-wasm-js test",
        "(cd packages/rava-wasm-js && npm pack --dry-run)",
        "master CI run `26017062950`",
        "Pages run `26017062966`",
        "No external security review has been completed",
    ] {
        assert!(
            notes.contains(required),
            "R2 review candidate notes missing: {required}"
        );
    }
...
fn v1_preview_surface_docs_pin_cli_and_json_shapes() -> Result<(), Box<dyn Error>> {
    ...
    for required in [
        ...
        "does not add dynamic key discovery",
        "resolver selection",
        "fresh_until_unix",
...
fn v1_preview_migration_notes_cover_preview_surface_changes() -> Result<(), Box<dyn Error>> {
    ...
    for required in [
        ...
        "--require-fresh-trust-bundle",
        "rava-static-trust-bundle-v0",
        "does not add dynamic resolver selection",
        "fresh_until_unix",
        "not distributed revocation publication or outage handling",
        "rava-wasm",
        "rava-wasm-js",
        "docs/protocol/v1-preview-surface.md",
        "not production-ready security software",
...
fn production_trust_docs_define_external_operational_requirements() -> Result<(), Box<dyn Error>> {
    ...
    for required in [
        ...
        "--require-metrics",
        "--require-fresh-trust-bundle",
        "Confirm `GET /healthz` reports the required guardrails",
        "Export the local audit metadata with `rava audit export --output",
        "Capture `GET /metrics` output without raw action payloads",
        "not production key custody, distributed replay, distributed revocation, managed audit storage, production caller identity, distributed rate limiting, managed monitoring, or external security review evidence",
    ...
    let runbooks: [(&str, &[&str]); 5] = [
        (
            ...
            &[
                ...
                "Generate a replacement key with `rava key generate",
                "Update the explicit static trust bundle",
                "Verify newly signed objects with the updated trust bundle",
                "revoke the old signer ID with `rava key revoke",
                "not a production rotation ceremony, custody-system test, trust-bundle publication system, rollback test, or operator approval workflow",
        ...
        (
            ...
            &[
                ...
                "fresh_until_unix",
                "--require-fresh-revocations",
                "updates local file-backed snapshots under a lock file",
                "not a revocation distribution system",
                "It does not provide revocation publication, network distribution, cache invalidation, emergency propagation, cross-node freshness, or outage handling.",
        ...
        (
            ...
            &[
                ...
                "GET /metrics",
                "Prometheus-style",
                "process-local and metadata-only",
                "not managed dashboards, alerting, incident response, long-term retention, cross-node aggregation",
                "replay attempts",
